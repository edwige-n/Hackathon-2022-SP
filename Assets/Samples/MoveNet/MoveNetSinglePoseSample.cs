...
public class MoveNetSinglePoseSample : MonoBehaviour
{
    ...
    private void DrawResult(MoveNet.Result[] results)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            Vector3 monVecteur; 
            Vector3 monVecteur2; 
            monVecteur = new Vector3(a.x, 1f - a.y, 0); 
            monVecteur2 = new Vector3(b.x, 1f - b.y, 0); 

            draw.Point(MathTF.Lerp(min, max, monVecteur), 5);
            draw.Point(MathTF.Lerp(min, max, monVecteur2), 5);
